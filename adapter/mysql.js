...
getData = (message = '', callback) => connection.query(
    `SELECT * FROM ${DATABASE_NAME}.whatsapp_initial WHERE keywords LIKE '%${message}%'  LIMIT 1`,
    (error, results
    ) => {
...
getReply = (option_key = '', callback) => connection.query(
    `SELECT * FROM ${DATABASE_NAME}.whatsapp_response WHERE option_key = '${option_key}'  LIMIT 1`,
    (error, results
    ) => {
        const [response] = results;
...
getMessages = (number) => new Promise((resolve, reejct) => {
    try {
        connection.query(
            `SELECT * FROM ${DATABASE_NAME}.whatsapp_response WHERE number = '${number}'`, (error, results) => {
                if (error) {
                    console.log(error)
                }
                const [response] = results;
...
saveMessageMysql = (message, date = null, trigger, number = null) => new Promise((resolve, reejct) => {
    try {
        console.log("date->"+date);
        if (date == null ||!date) { date = new Date().toISOString(); }
        console.log("date->"+date);
        if (number == null) {
            number = trigger.replace('@c.us', '');
        }
        connection.query(
            `INSERT INTO ${DATABASE_NAME}.whatsapp_messages  ` + "( `message`, `date`, `trigger`, `number`)" + ` VALUES ('${message}','${date}','${trigger}', '${number}')`, (error, results) => {
                if (error) {
