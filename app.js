...
/**
 * ?Crear  QR y envio texto
 */
app.use('/qrcode', getQr)
app.post('/sendMessage', (req, res) => {
    console.log('Enviando mensaje mediante POST')
    //const { message, number } = req.body
    const { message, number } = req.query
    if (!client) {
        res.send({ status: 'El objeto cliente no está definido.' })
        return console.error("El objeto cliente no está definido.");
    }
    if (!message && message == undefined) {
        res.send({ status: 'error en lo parametro message' })
        return console.error("error en lo parametro message");
    }
    if (!number && number == undefined) {
        res.send({ status: 'error en lo parametro number' })
        return console.error("error en lo parametro number");
    }
